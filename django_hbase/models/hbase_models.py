...
class HBaseModel:
    ...
    def init_from_row(cls, row_key, row_data):
        ...
        data = cls.deseiralize_row_key(row_key)
    ...
    def deserialize_field(cls, key, value):
        ...
        if field.reverse:
            value = value[::-1]
        if field.field_type in [IntegerField, TimestampField]:
            return int(value)
        return value

    @classmethod
    def serialze_row_data(cls, data):
        row_data = {}
        field_hash = cls.get_field_hash()
        for key, field in field_hash.items():
            ...
            if column_value is None:
                continue
            row_data[column_key] = cls.deserialize_field(field, column_value)
        return row_data

    def save(self):
        row_data = self.serialze_row_data(self.__dict__)
