...
class NewsFeedService(object):
    ...
    def fanout_to_followers(cls, tweet):
        ...
        # worker 进程中会执行 fanout_newsfeeds_task 里的代码来是实现一个异步的任务处理。
        # 如果这个任务需要处理 10s 则这 10s 会花费在 worker 进程上，而不是花费在用户发 tweet
        # 的进程中。所以这里 .delay 操作会马上执行马上结束从而不影响用户的正常操作。
        # （因为i这里只是创建了一个任务，把任务信息放在了 message queue 里，并没有真正执行这个函数）
        # 要注意的是，delay 里的参数必须是可以被 celery serialize 的值，因为 worker 进程是一个独立
        # 的进程，甚至在不同的机器上，没有办法知道当前 web 进程的某片内存空间里的只是什么。所以
        # 我们只能把 tweet.id 作为参数传递进去，而不能把 tweet 传进去。因为 celery 并不知道如何
        # serialize Tweet。
