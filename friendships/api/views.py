...
from friendships.api.serializers import (
    FollowerSerializer,
    FollowingSerializer,
    FriedshipSerializerForCreate,
)
from friendships.models import Friendship


class FriendshipViewSet(viewsets.GenericViewSet):
    # 我们希望 POST /api/friendship/1/follow 是去 follow_user_id=1 的用户
    ...
    def follow(self, request, pk):
        ...
        serializer = FriedshipSerializerForCreate(data={
    ...
    def unfollow(self, request, pk):
        ...
        # 为什么会出现多种类型数据的删除？因为可能因为 foreighn key 设置了 cascade 出现级联
        # 删除，也就是比如 A model 的某个属性是 B model 的 foreign key, 并且设置了
        # on_delete = models.CASCADE，那么当 B 的某个数据被删除的时候，A中的关联也会被删除。
        # 所以 CASCADE 是很危险的，我们一般最好不用，而是用 on_delete=models.SET_NULL
        # 取而代之，这样至少可以避免误删操作带来的多米诺效应。
